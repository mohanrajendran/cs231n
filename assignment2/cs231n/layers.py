...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, x_normal, sample_mean, sample_var, gamma, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    x, x_normal, sample_mean, sample_var, gamma, eps = cache
    ...
    dsample_var = np.sum(dx_normal * (x - sample_mean) * -0.5 * (sample_var + eps)**-1.5, axis=0)
    dsample_mean = np.sum(dx_normal / -np.sqrt(sample_var + eps), axis=0) + dsample_var * -2 * np.average(x - sample_mean, axis=0)
    
    dx = dx_normal / np.sqrt(sample_var + eps) + dsample_var * 2 * (x - sample_mean) / m + dsample_mean / m
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    ###########################################################################
    # From https://costapt.github.io/2016/07/09/batch-norm-alt/
    x, x_normal, sample_mean, sample_var, gamma, eps = cache
    ...
    dx = gamma * (m*dout - dgamma * x_normal - dbeta) / (m * np.sqrt(sample_var+eps))
    ##############################v#############################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_p = x.reshape(N, G, C//G, H, W)
    group_mean = np.mean(x_p, axis=(2, 3, 4), keepdims=True)
    group_var = np.var(x_p, axis=(2,3,4), keepdims=True)
    
    x_normal = (x_p - group_mean) / np.sqrt(group_var + eps)
    x_normal = x_normal.reshape(N, C, H, W)
    
    out = gamma * x_normal + beta
    cache = (x, x_normal, group_mean, group_var, gamma, G, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    x, x_normal, group_mean, group_var, gamma, G, eps = cache
    D = H * W * C // G
    ...
    dx_normal.resize(N, G, C // G, H, W)
    x_normal.resize(N, G, C // G, H, W)

    dx = ((1 / np.sqrt(group_var + eps) / D) *
          (D * dx_normal -
           np.sum(dx_normal, axis=(2, 3, 4), keepdims=True) -
           x_normal * np.sum(dx_normal * x_normal, axis=(2, 3, 4), keepdims=True)))
